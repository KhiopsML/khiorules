...
KRDRPredictorRule::KRDRPredictorRule()
{
	// Initialisation des parametres de la regle et de ses operandes
	SetName("Rule-Based Classifier");
	SetLabel("Classifieur a base de regles");
}
...
KWDerivationRule* KRDRPredictorRule::Create() const
{
	return new KRDRPredictorRule;
}

Object* KRDRPredictorRule::ComputeStructureResult(const KWObject* kwoObject) const
{
	int nOperand;
...
void KRDRPredictorRule::ComputeTargetProbs() const
{
	int nTarget;
	int nFrequencyMax;
...
void KRDRPredictorRule::Compile(KWClass* kwcOwnerClass)
{
	...
	targetFrequencies = cast(const KWDRFrequencies*,
		targetDataGrid->GetOperandAt(1)->GetReferencedDerivationRule(kwcOwnerClass));
